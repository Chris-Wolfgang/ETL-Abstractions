...
/// <typeparam name="TProgress">The type of the progress report object produced during loading</typeparam>
/// <remarks>
...
/// any exceptions that may occur during the loading process, including but not
...
/// transient failures. The loaded data is provided as an
...
/// may handle the conversion of objects of type TDestination to strings (rows). In this case,
/// the pragmatic approach is to allow the loader to perform this minimal transformation.
/// </remarks>
public interface ILoadWithProgressAndCancellationAsync<in TDestination, out TProgress> :
    ...
    where TDestination : notnull where TProgress : notnull
{
    ...
    /// <remarks>
    /// The loader should be able to handle cancellation requests gracefully.
    /// If the caller doesn't plan on cancelling the loading, they can pass CancellationToken.None.
    /// </remarks>
